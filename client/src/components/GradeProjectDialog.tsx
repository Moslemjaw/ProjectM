...
import { Building2, DollarSign, Loader2, User, Calendar, Clock, FileText, Users, AlertCircle, Download } from "lucide-react";
...
export function GradeProjectDialog({ project, open, onOpenChange }: GradeProjectDialogProps) {
  ...
  const [savedSignature, setSavedSignature] = useState<string | undefined>(existingGrade?.signature ?? undefined);
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      criteria: {
        backgroundIntroduction: existingGrade?.criteria?.backgroundIntroduction || 0,
        noveltyOriginality: existingGrade?.criteria?.noveltyOriginality || 0,
        clearRealisticObjectives: existingGrade?.criteria?.clearRealisticObjectives || 0,
        disseminationResults: existingGrade?.criteria?.disseminationResults || 0,
        significance: existingGrade?.criteria?.significance || 0,
        feasibilityPlanning: existingGrade?.criteria?.feasibilityPlanning || 0,
      },
      criteriaComments: {
        backgroundIntroduction: existingGrade?.criteriaComments?.backgroundIntroduction || "",
        noveltyOriginality: existingGrade?.criteriaComments?.noveltyOriginality || "",
        clearRealisticObjectives: existingGrade?.criteriaComments?.clearRealisticObjectives || "",
        disseminationResults: existingGrade?.criteriaComments?.disseminationResults || "",
        significance: existingGrade?.criteriaComments?.significance || "",
        feasibilityPlanning: existingGrade?.criteriaComments?.feasibilityPlanning || "",
      },
  ...
  const totalScore = criteriaValues 
    ? Object.values(criteriaValues).reduce((sum, val) => sum + (Number(val) || 0), 0)
    : 0;
  ...
  const gradeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: isAlreadyGraded ? "Grade updated successfully!" : "Project graded successfully!",
      });
      onOpenChange(false);
    },
    onError: async (error: Error) => {
      ...
      try {
        const errorResponse = await fetch(`/api/projects/${project.id}/grade`, {
          method: 'POST',
        ...
        if (!errorResponse.ok) {
          ...
          if (errorData.message === "INCOMPLETE_PROFILE") {
        ...
        // If error parsing fails, show regular toast
      }
        ...
        <AlertDialogContent data-testid="alert-incomplete-profile">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3">
              <p>
                You need to complete your profile information before submitting a grade.
                This information will be included in the printed evaluation forms.
              </p>
          ...
          <TabsContent value="details" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground mb-1">Abstract</h3>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">{project.budget} KD</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                    <span className="text-sm text-muted-foreground">Submitted:</span>
                    <span className="text-sm font-medium">{format(new Date(project.createdAt), 'MMM dd, yyyy')}</span>
                  </div>
                  {project.duration && (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {project.fileUrls && project.fileUrls.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-muted-foreground mb-2">Complete Proposal</h3>
                  ...
                  {project.researchFormUrls && project.researchFormUrls.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-muted-foreground mb-2">Research Project Form</h3>
                      <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        {project.submitter.academicLevel && (
                          <div>
                            ...
                            <span className="font-medium">{project.submitter.academicLevel}</span>
                          </div>
                        )}
                        {project.submitter.college && (
                          <div>
                            <span className="text-muted-foreground">College:</span>{' '}
                            <span className="font-medium">{project.submitter.college}</span>
                          </div>
                        )}
                        {project.submitter.specialty && (
                          <div className="col-span-2">
                            <span className="text-muted-foreground">Specialty:</span>{' '}
                            <span className="font-medium">{project.submitter.specialty}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <TabsContent value="grade">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">Research Project Evaluation Form</CardTitle>
                    <CardDescription>
                      Grade each criterion out of 10 points and provide detailed comments
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-4 bg-primary/10 border border-primary/20 rounded-lg">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Your score (60%) + AI score (40%) = Final score (100%)</p>
                      </div>
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">1. Background/Introduction Section</h3>
                      <p className="text-sm text-muted-foreground">
                        • Is the introduction of the research area sufficiently explained?<br/>
                        • Are the limitations of existing work clearly highlighted?<br/>
                        • Does the section provide a clear summary of the major components?
                      </p>
                      <FormField
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">2. Novelty and Originality</h3>
                      <p className="text-sm text-muted-foreground">
                        • Is the methodology clearly defined and structured?<br/>
                        • Does the proposed activity suggest creative and original concepts?<br/>
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">3. Clear and Realistic Objectives</h3>
                      <p className="text-sm text-muted-foreground">
                        • Are the research objectives clearly stated?<br/>
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">4. Dissemination of Research Results</h3>
                      <p className="text-sm text-muted-foreground">
                        • Are the expected outputs (e.g., journal/conference publications) clearly identified?<br/>
                        • Is the number of proposed publications reasonable?<br/>
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">5. Significance of the Research</h3>
                      <p className="text-sm text-muted-foreground">
                        • Does the research address a significant problem?<br/>
                        • Does it have the potential for academic, social, or economic impact?<br/>
                    ...
                    <div className="space-y-3 p-4 border rounded-lg">
                      <h3 className="font-semibold text-base">6. Feasibility and Planning</h3>
                      <p className="text-sm text-muted-foreground">
                        • Is the timeline realistic and well-structured?<br/>
                        • Does the proposal demonstrate the ability to complete the project successfully?<br/>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Provide overall feedback on the project strengths and weaknesses
                          </FormDescription>
                          <FormControl>
                            <Textarea
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Suggest improvements or future directions for the research
                          </FormDescription>
                          <FormControl>
                            <Textarea
                    ...
                    <div className="space-y-3">
                      ...
                      {/* Signature Mode Tabs */}
                      <Tabs value={signatureMode} onValueChange={(v) => setSignatureMode(v as 'draw' | 'upload')} className="w-full">
                        <TabsList className="grid w-full grid-cols-2">
                          <TabsTrigger value="draw" data-testid="tab-draw-signature">Draw Signature</TabsTrigger>
                          <TabsTrigger value="upload" data-testid="tab-upload-signature">Upload Signature</TabsTrigger>
                        </TabsList>

                        <TabsContent value="draw" className="space-y-3">
                          <div className="border-2 border-dashed rounded-lg p-2 bg-background" data-testid="signature-canvas-wrapper">
                            <SignatureCanvas
                              ref={signatureRef}
                              canvasProps={{
                ...
                <DialogFooter className="flex justify-between items-center">
                  <div className="text-sm text-muted-foreground">
                    {isAlreadyGraded && (
                      <span className="text-amber-600 dark:text-amber-500">You have already graded this project. Submitting will update your grade.</span>
                    )}
                  </div>
