...
import { FileText, Users, BarChart3, Zap, Sparkles, ArrowRight, GraduationCap, Building2, FlaskConical, BookOpen } from "lucide-react";
...
function SharpSquare({ size = 40, color = "navy", className = "" }: { size?: number; color?: "navy" | "orange" | "teal"; className?: string }) {
...
function PlusShape({ size = 80, className = "" }: { size?: number; className?: string }) {
  const blockSize = size / 2;
  return (
    <div className={`relative ${className}`} style={{ width: size, height: size }}>
      <div style={{ position: "absolute", top: 0, right: 0, width: blockSize, height: blockSize, backgroundColor: navyBlue }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, width: blockSize, height: blockSize, backgroundColor: navyBlue }} />
    </div>
  );
}

function DiagonalSquares({ size = 70, className = "" }: { size?: number; className?: string }) {
  const blockSize = size / 2;
  return (
    <div className={`relative ${className}`} style={{ width: size, height: size }}>
      <div style={{ position: "absolute", top: 0, left: 0, width: blockSize, height: blockSize, backgroundColor: navyBlue }} />
      <div style={{ position: "absolute", bottom: 0, right: 0, width: blockSize, height: blockSize, backgroundColor: navyBlue }} />
    </div>
  );
}

function Diamond({ color, size = 30, className = "" }: { color: "teal" | "orange" | "navy"; size?: number; className?: string }) {
...
function Triangle({ size = 30, color = "teal", direction = "down", className = "" }: { size?: number; color?: "teal" | "navy" | "orange"; direction?: "down" | "up" | "left" | "right"; className?: string }) {
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          >
            ...
            <div className="flex flex-col">
              <h1 className="text-lg font-bold leading-tight" style={{ color: "#0e1c43" }}>
                AASU
              </h1>
              <span className="text-xs text-muted-foreground">Research Portal</span>
            </div>
          </motion.div>
          <motion.div
            initial={{ x: 20, opacity: 0 }}
      ...
      <section className="container mx-auto px-4 py-12 md:py-20 relative">
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#60c2ac]/30" style={{ backgroundColor: "rgba(96, 194, 172, 0.1)" }}>
              <Sparkles className="h-4 w-4" style={{ color: "#60c2ac" }} />
              <span className="text-sm font-medium" style={{ color: "#60c2ac" }}>AI-Powered Research Management</span>
            </div>
          </motion.div>
          ...
          >
            Fostering innovation in education and research through streamlined project proposal submission, peer review, and AI-assisted evaluation
          </motion.p>
          
          <motion.div variants={itemVariants} className="pt-4 flex gap-4 justify-center flex-wrap">
            <Button size="lg" asChild data-testid="button-get-started" className="shadow-lg group">
              <a href="/login">
                Get Started
                <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </a>
            </Button>
            <Button size="lg" variant="outline" className="shadow-md" data-testid="button-learn-more">
              Learn More
            </Button>
          </motion.div>
        </motion.div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">
              Supporting Kuwait Vision 2035 through world-class research management
            </p>
          </motion.div>
            ...
            {[
              {
                ...
                description: "Submit research proposals with complete documentation and file uploads",
                color: "#0e1c43",
                delay: 0
              },
              {
                icon: Zap,
                title: "AI Evaluation",
                description: "Automated AI assessment for relevance, clarity, and institutional alignment",
                color: "#fbb216",
                delay: 0.1
              },
              {
                icon: Users,
                title: "Expert Review",
                description: "Multi-reviewer evaluation with detailed feedback and comprehensive grading",
                color: "#60c2ac",
                delay: 0.2
              },
              {
                ...
                description: "Real-time insights and reporting for administrators and editors",
                color: "#0e1c43",
                delay: 0.3
              }
            ].map((feature, index) => (
      ...
      <section className="container mx-auto px-4 py-16 relative">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <h3 className="text-3xl font-bold text-center mb-4">Research Centers</h3>
            <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: FlaskConical, name: "Data Science & AI", color: "#60c2ac" },
              ...
              >
                <center.icon className="h-8 w-8" style={{ color: center.color }} />
                <span className="text-sm font-medium text-center">{center.name}</span>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            {[
              {
                ...
                description: "Faculty members submit research proposals with detailed descriptions, budgets, timelines, and supporting documents"
              },
              {
                number: 2,
                title: "AI-Powered Analysis",
                description: "Automated evaluation assesses project alignment with university goals, clarity, and feasibility (40% weight)"
              },
              {
                number: 3,
                title: "Expert Peer Review",
                description: "Multiple expert reviewers evaluate and grade your project based on comprehensive criteria (60% weight)"
              },
              {
                number: 4,
                title: "Receive Results",
                description: "Get comprehensive feedback, detailed recommendations, and your final evaluation score"
              }
            ].map((step, index) => (
              <motion.div
                key={index}
              ...
              >
                <div 
                  ...
                  style={{ background: "linear-gradient(135deg, #0e1c43 0%, #60c2ac 100%)" }}
                >
                  {step.number}
                </div>
                <div className="flex-1">
                  <h4 className="font-semibold text-lg mb-1 group-hover:text-[#60c2ac] transition-colors">{step.title}</h4>
      ...
      <footer className="border-t mt-16 backdrop-blur-sm bg-background/80">
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
            ...
            <p>&copy; {new Date().getFullYear()} AASU Research Portal. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
