...
import { setupSession, isAuthenticated, hashPassword, verifyPassword, generateUserId, type AuthRequest } from "./auth";
import { insertProjectSchema, projectSubmissionSchema, insertGradeSchema, insertUserSchema, updateUserSchema, loginSchema, USER_ROLES } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/users", isAuthenticated, isAdmin, async (req: AuthRequest, res) => {
    try {
  ...
  app.patch("/api/users/:id", isAuthenticated, isAdmin, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      ...
      const { id: _, createdAt: __, ...safeUpdates } = validatedUpdates as any;
  ...
  app.post("/api/files/upload", isAuthenticated, async (req: AuthRequest, res) => {
    try {
  ...
  app.get("/api/files/:fileId", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      ...
      if (!isOwner) {
        ...
        for (const project of projects) {
          ...
          if (usesFile) {
            ...
            if (currentUser?.role === USER_ROLES.FACULTY && project.submitterId === userId) {
            ...
            if (currentUser?.role === USER_ROLES.REVIEWER) {
              const assignments = await storage.getReviewerAssignments(project.id);
              const hasAssignment = assignments.some(a => a.reviewerId === userId);
              if (hasAssignment) {
                hasProjectAccess = true;
                break;
              }
  ...
  app.post("/api/projects", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      ...
      if (currentUser?.role !== USER_ROLES.FACULTY) {
        return res.status(403).json({ message: "Only faculty can submit projects" });
      }
      ...
      (async () => {
        try {
          ...
          await storage.updateProjectAIScore(project.id, normalizedScore.toString());
          await storage.updateProject(project.id, {
            aiFeedback: aiResult.feedback,
            alignedCenter: aiResult.alignedCenter,
          });
          await storage.updateProjectStatus(project.id, "pending_editor_review");
          ...
          await storage.createNotification({
            ...
            message: `Your project "${project.title}" has been evaluated by AI. Score: ${normalizedScore.toFixed(1)}/40${aiResult.alignedCenter ? `. Aligned with: ${aiResult.alignedCenter}` : ''}`,
          });
          ...
          await storage.logActivity({
            ...
            userId: project.submitterId,
            action: "ai_grade",
            details: `AI grading completed automatically with score ${normalizedScore.toFixed(1)}/40`,
          });
        } catch (error) {
          ...
          try {
            ...
            console.error("Failed to recover from AI grading error:", recoveryError);
          }
        }
      })();
  ...
  app.get("/api/projects/my", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
  ...
  app.post("/api/projects/:id/resubmit", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      ...
      if (currentUser?.role !== USER_ROLES.FACULTY) {
        return res.status(403).json({ message: "Only faculty can resubmit projects" });
      }
      ...
      if (project.submitterId !== userId) {
        return res.status(403).json({ message: "You can only resubmit your own projects" });
      }

      if (project.status !== "needs_revision") {
        return res.status(400).json({ message: "Only projects with revision requests can be resubmitted. Rejected projects are final." });
      }
      ...
      if (req.body.alignedCenter !== undefined) updateData.alignedCenter = req.body.alignedCenter;
      if (req.body.fileUrls) updateData.fileUrls = req.body.fileUrls;
      if (req.body.researchFormUrls) updateData.researchFormUrls = req.body.researchFormUrls;
      ...
      (async () => {
        try {
          ...
          await storage.updateProjectAIScore(updatedProject.id, normalizedScore.toString());
          await storage.updateProject(updatedProject.id, {
            aiFeedback: aiResult.feedback,
            alignedCenter: aiResult.alignedCenter,
          });
          await storage.updateProjectStatus(updatedProject.id, "pending_editor_review");

          await storage.createNotification({
            userId: updatedProject.submitterId,
            projectId: updatedProject.id,
            title: "Resubmission Evaluated",
            message: `Your resubmitted project "${updatedProject.title}" has been evaluated by AI with a score of ${normalizedScore.toFixed(1)}/40. It is now pending editor review.`
          });
          ...
          await storage.logActivity({
            ...
            details: `Resubmitted project "${updatedProject.title}" graded by AI: ${normalizedScore.toFixed(1)}/40`
          });
        } catch (aiError) {
  ...
  app.get("/api/projects/all", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const currentUser = await storage.getUser(userId);
      console.log("[/api/projects/all] Current user:", currentUser?.email, currentUser?.role);
      ...
      if (currentUser.role !== USER_ROLES.EDITOR) {
        console.log("[/api/projects/all] Forbidden: User is not editor, role is:", currentUser.role);
        return res.status(403).json({ message: "Forbidden - Editor role required", userRole: currentUser.role });
      }
  ...
  app.get("/api/projects/assigned", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);

      if (currentUser?.role !== USER_ROLES.REVIEWER) {
        return res.status(403).json({ message: "Only reviewers can view assigned projects" });
      }

      const projects = await storage.getProjectsByReviewer(userId);
      console.log(`[DEBUG] Found ${projects.length} projects for reviewer ${userId}`);
      ...
      const assignedProjects = projects.filter(p => {
        const hasValidAssignment = p.reviewerAssignments?.some((a: any) => {
          console.log(`[DEBUG] Project ${p.id} assignment:`, { reviewerId: a.reviewerId, status: a.status, match: a.reviewerId === userId && a.status !== 'rejected' });
          return a.reviewerId === userId && a.status !== 'rejected';
        });
        return hasValidAssignment;
      });
      
      console.log(`[DEBUG] Returning ${assignedProjects.length} assigned projects`);
  ...
  app.get("/api/projects/available", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);

      if (currentUser?.role !== USER_ROLES.REVIEWER) {
        return res.status(403).json({ message: "Only reviewers can view available projects" });
      }
  ...
  app.get("/api/projects/pending-decisions", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);

      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can access this endpoint" });
      }

      // Use storage layer to get all projects, then filter by status
      const allProjects = await storage.getAllProjects();
      const pendingDecisions = allProjects.filter(p => p.status === "pending_final_decision");
  ...
  app.get("/api/projects/:id", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      ...
      if (!project) {
        console.log("[/api/projects/:id] Project not found for ID:", projectId);
        return res.status(404).json({ message: "Project not found" });
      }
  ...
  app.get("/api/projects", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can view all projects" });
      }
  ...
  app.post("/api/projects/:id/accept", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const projectId = req.params.id;
      const currentUser = await storage.getUser(userId);
      ...
      if (currentUser?.role !== USER_ROLES.REVIEWER) {
        return res.status(403).json({ message: "Only reviewers can accept assignments" });
      }
      ...
      if (project.status !== 'pending_assignment') {
        return res.status(400).json({ message: "This project is no longer available for assignment" });
      }

      // Check if editor has already responded to this project
      const assignments = await storage.getReviewerAssignments(projectId);
      const editorAssignment = assignments.find(a => a.reviewerId === userId);
      
      if (editorAssignment) {
        if (editorAssignment.status === 'accepted') {
          return res.status(400).json({ message: "You have already accepted this assignment" });
        }
        if (editorAssignment.status === 'rejected') {
          return res.status(400).json({ message: "You have already rejected this assignment" });
        }
      ...
      const updatedAssignments = await storage.getReviewerAssignments(projectId);

      const requiredEditors = Number(project.budget) <= 20000 ? 2 : 3;
      const acceptedCount = updatedAssignments.filter((a: any) => a.status === 'accepted').length;
  ...
  app.post("/api/projects/:id/reject", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const projectId = req.params.id;
      const currentUser = await storage.getUser(userId);
      ...
      if (currentUser?.role !== USER_ROLES.REVIEWER) {
        return res.status(403).json({ message: "Only reviewers can reject assignments" });
      }
      ...
      if (editorAssignment) {
        if (editorAssignment.status === 'accepted') {
          return res.status(400).json({ message: "You have already accepted this assignment and cannot reject it" });
        }
        if (editorAssignment.status === 'rejected') {
          return res.status(400).json({ message: "You have already rejected this assignment" });
        }
  ...
  app.get("/api/editor/projects", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
  ...
  app.post("/api/editor/projects/:id/accept", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      const projectId = req.params.id;
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can accept projects" });
      }
      ...
      if (project.status !== "pending_editor_review") {
        return res.status(400).json({ message: "Project is not pending editor review" });
      }
      ...
      const selectedReviewers = allReviewers.slice(0, requiredReviewers).map(r => r.id);
      ...
      res.json({ message: "Project accepted and reviewers assigned successfully" });
    } catch (error) {
  ...
  app.post("/api/editor/projects/:id/reject", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      const projectId = req.params.id;
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can reject projects" });
      }
  ...
  app.post("/api/editor/projects/:id/request-revision", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      const projectId = req.params.id;
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can request revisions" });
      }
  ...
  app.get("/api/editor/projects/under-review", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
  ...
  app.post("/api/editor/projects/:id/assign-reviewers", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      const projectId = req.params.id;
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can assign reviewers" });
      }

      if (!Array.isArray(reviewerIds) || reviewerIds.length === 0) {
        return res.status(400).json({ message: "At least one reviewer must be assigned" });
      }
      ...
      if (reviewerIds.length !== requiredReviewers) {
        return res.status(400).json({
          message: `This project requires exactly ${requiredReviewers} reviewers (budget ${budget <= 20000 ? '≤' : '>'} 20,000 KD). You selected ${reviewerIds.length} reviewers.`
        });
      }
  ...
  app.get("/api/users/reviewers", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can access reviewer list" });
      }
  ...
  app.post("/api/users", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can create users" });
      }
  ...
  app.get("/api/activity-logs", isAuthenticated, isAdmin, async (req: AuthRequest, res) => {
    try {
  ...
  app.post("/api/projects/:id/assign", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!Array.isArray(reviewerIds) || reviewerIds.length === 0) {
        return res.status(400).json({ message: "At least one editor must be assigned" });
      }
      ...
      if (reviewerIds.length !== requiredEditors) {
        return res.status(400).json({
          message: `This project requires exactly ${requiredEditors} editors (budget ${budget <= 20000 ? '≤' : '>'} 20,000 KD). You selected ${reviewerIds.length} editors.`
        });
      }
  ...
  app.patch("/api/projects/:id/status", isAuthenticated, isAdmin, async (req: AuthRequest, res) => {
    try {
      const projectId = req.params.id;
      ...
      const validStatuses = ["pending_ai", "pending_assignment", "under_review", "graded"];
  ...
  app.patch("/api/projects/:id", isAuthenticated, isAdmin, async (req: AuthRequest, res) => {
    try {
      const projectId = req.params.id;
  ...
  app.post("/api/projects/:id/ai-grade", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user!.id;
      const projectId = req.params.id;
      const project = await storage.getProject(projectId);
      ...
      await storage.updateProjectAIScore(projectId, normalizedScore.toString());
      ...
      await storage.logActivity({
        ...
        userId,
        action: "ai_grade",
        details: `AI grading completed with score ${normalizedScore.toFixed(1)}/40`,
      });
  ...
  app.post("/api/projects/:id/grade", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (currentUser?.role !== USER_ROLES.REVIEWER) {
        return res.status(403).json({ message: "Only reviewers can submit grades" });
      }
      ...
      if (missingFields.length > 0) {
        return res.status(400).json({ 
          ...
          missingFields,
          details: "Please complete your profile information before submitting a grade."
        });
      }
      ...
      const isAssigned = project.reviewerAssignments.some((a) => a.reviewerId === userId);
      if (!isAssigned) {
      ...
      if (allGrades.length === requiredEditors) {
        ...
        const aiScore = typeof project.aiScore === 'string' 
          ? parseFloat(project.aiScore) 
          : Number(project.aiScore) || 0;
        
        const editorScoresSum = allGrades.reduce((sum, g) => {
          const score = typeof g.score === 'string' ? parseFloat(g.score) : Number(g.score);
          return sum + score;
        }, 0);
        ...
        await storage.updateProjectStatus(projectId, 'pending_final_decision');
  ...
  app.get("/api/projects/:projectId/grades/:gradeId/print", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      ...
      if (currentUser?.role === USER_ROLES.FACULTY) {
        ...
        const submitterId = typeof project.submitterId === 'object' && project.submitterId?.id 
          ? project.submitterId.id 
          : project.submitterId;
        
        if (submitterId !== userId) {
          return res.status(403).json({ message: "You can only view evaluation forms for your own projects" });
        }
      } else if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors and faculty can view evaluation forms" });
      }
      ...
      const logoPath = path.join(process.cwd(), 'server', 'assets', 'university-logo.jpg');
      const addLogo = () => {
        try {
      ...
      doc.fontSize(16).font('Helvetica-Bold')
         .text('Research Project Proposal Evaluation Form', { align: 'center' })
         .fontSize(16).text('(External Evaluator)', { align: 'center' })
         .fontSize(14).text('Total Score 60', { align: 'center' })
         .moveDown(2);
      ...
      const drawTable = (x: number, y: number, headers: string[], rows: string[][], colWidths: number[]) => {
        ...
        colWidths.forEach((width, i) => {
          ...
          doc.text(headers[i], currentX + 5, currentY + 5, { width: width - 10, align: 'left' });
        });
        currentY += rowHeight;
        ...
        rows.forEach(row => {
          colWidths.forEach((width, i) => {
            const currentX = x + colWidths.slice(0, i).reduce((a, b) => a + b, 0);
            doc.rect(currentX, currentY, width, rowHeight).stroke();
            doc.text(row[i] || '', currentX + 5, currentY + 5, { width: width - 10, align: 'left' });
          });
          currentY += rowHeight;
        });
      ...
      yPos = drawTable(tableX, yPos, ['Field', 'Value'], [
      ...
      const evaluationCriteria = [
        {
          ...
          details: ['• Is the introduction of the research area sufficiently explained?', '• Are the limitations of existing work clearly highlighted?', '• Does the section provide a clear summary of the major components of the research?'],
          weight: '10',
        ...
        },
        {
          ...
          details: ['• Is the methodology clearly defined and structured?', '• Does the proposed activity suggest creative and original concepts?', '• Are the steps and tasks in the methodology logical and well-detailed?'],
          weight: '10',
        ...
        },
        {
          ...
          details: ['• Are the research objectives clearly stated?', '• Are the objectives realistic and aligned with the project duration?'],
          weight: '10',
        ...
        },
        {
          ...
          details: ['• Are the expected outputs (e.g., journal/conference publications) clearly identified?', '• Is the number of proposed publications reasonable?', '• Are the target venues for publication of acceptable quality'],
          weight: '10',
        ...
        },
        {
          ...
          details: ['• Does the research address a significant problem?', '• Does it have the potential for academic, social, or economic impact?', '• Is the research aligned with international standards?'],
          weight: '10',
        ...
        },
        {
          ...
          details: ['• Is the timeline realistic and well-structured?', '• Does the proposal demonstrate the ability to complete the project successfully?', '• Is the budget appropriate and justified?'],
          weight: '10',
      ...
      const col3Width = evalTableWidth * 0.10;  // Weightage - 10%
      const col4Width = evalTableWidth * 0.10;  // Score - 10%
      const col5Width = evalTableWidth * 0.40;  // Comments - 40%
      ...
      doc.rect(col1, yPos, col1Width, rowHeight).stroke().text('S#', col1 + 2, yPos + 5, { width: col1Width - 4 });
      doc.rect(col2, yPos, col2Width, rowHeight).stroke().text('Criteria for\nEvaluation', col2 + 2, yPos + 5, { width: col2Width - 4 });
      doc.rect(col3, yPos, col3Width, rowHeight).stroke().text('Weightage', col3 + 2, yPos + 5, { width: col3Width - 4 });
      doc.rect(col4, yPos, col4Width, rowHeight).stroke().text("Reviewer's\nscore", col4 + 2, yPos + 5, { width: col4Width - 4 });
      doc.rect(col5, yPos, col5Width, rowHeight).stroke().text('Comments', col5 + 2, yPos + 5, { width: col5Width - 4 });
      ...
      const drawCriterion = (crit: typeof evaluationCriteria[0]) => {
        ...
        // Fill content
        doc.font('Helvetica-Bold').fontSize(8).text(crit.num, col1 + 2, yPos + 12, { width: col1Width - 4, align: 'center' });
        
        let textY = yPos + 12;
        doc.font('Helvetica-Bold').fontSize(8).text(crit.name, col2 + 2, textY, { width: col2Width - 4 });
        ...
        doc.fontSize(8).font('Helvetica-Bold').text(crit.weight, col3 + 2, yPos + estimatedHeight / 2 - 5, { width: col3Width - 4, align: 'center' });
        doc.text(String(crit.score), col4 + 2, yPos + estimatedHeight / 2 - 5, { width: col4Width - 4, align: 'center' });
        doc.font('Helvetica').text(crit.comment || '', col5 + 2, yPos + 12, { width: col5Width - 4 });
      ...
      const scoreCol1Width = tableWidth * 0.70;  // Description column
      ...
      doc.font('Helvetica-Bold').fontSize(10);
      doc.rect(scoreCol1, yPos, scoreCol1Width, 20).stroke().text('Overall Evaluator Score (out of 60)', scoreCol1 + 2, yPos + 5, { width: scoreCol1Width - 4 });
      doc.rect(scoreCol2, yPos, scoreCol2Width, 20).stroke().text('Total', scoreCol2 + 2, yPos + 5, { width: scoreCol2Width - 4, align: 'center' });
      doc.rect(scoreCol3, yPos, scoreCol3Width, 20).stroke().text('Obtained', scoreCol3 + 2, yPos + 5, { width: scoreCol3Width - 4, align: 'center' });
      
      yPos += 20;
      doc.font('Helvetica').fontSize(10);
      doc.rect(scoreCol1, yPos, scoreCol1Width, 20).stroke();
      doc.rect(scoreCol2, yPos, scoreCol2Width, 20).stroke().text('60', scoreCol2 + 2, yPos + 5, { width: scoreCol2Width - 4, align: 'center' });
      doc.rect(scoreCol3, yPos, scoreCol3Width, 20).stroke().text(String(totalScore), scoreCol3 + 2, yPos + 5, { width: scoreCol3Width - 4, align: 'center' });
      ...
      doc.font('Helvetica-Bold').fontSize(11);
      doc.text('Reviewer Information (Only visible to the University Administration)', 50, yPos, { 
        align: 'center',
      ...
      yPos = drawTable(tableX, yPos, ['Field', 'Value'], reviewerInfoRows, [tableWidth * 0.3, tableWidth * 0.7]);
      ...
      doc.text('Signature', tableX + 5, yPos + 5, { width: sigCol1Width - 10 });
      
      doc.rect(tableX + sigCol1Width, yPos, sigCol2Width, signatureRowHeight).stroke();
      ...
      if (grade.signature) {
        try {
          ...
          doc.font('Helvetica').fontSize(9);
          doc.text('[Digital signature captured]', tableX + sigCol1Width + 5, yPos + 5, { 
      ...
      res.setHeader("Content-Disposition", `attachment; filename="Evaluation_${project.title.replace(/[^a-z0-9]/gi, '_')}_${reviewer.firstName}_${reviewer.lastName}.pdf"`);
  ...
  app.post("/api/projects/:id/final-decision", isAuthenticated, async (req: AuthRequest, res) => {
    try {
      const userId = req.user!.id;
      const currentUser = await storage.getUser(userId);
      const projectId = req.params.id;
      ...
      if (currentUser?.role !== USER_ROLES.EDITOR) {
        return res.status(403).json({ message: "Only editors can make final decisions" });
      }

      if (!decision || !["accept", "reject", "revision"].includes(decision)) {
        return res.status(400).json({ message: "Invalid decision. Must be: accept, reject, or revision" });
      }
      ...
      if (project.status !== "pending_final_decision") {
        return res.status(400).json({ message: "Project is not pending final decision" });
      }
      ...
      if (decision === "accept") {
        ...
        notificationMessage = `Congratulations! Your project "${project.title}" has been accepted. View detailed reviewer feedback in your dashboard. ${comments || ""}`;
      } else if (decision === "reject") {
        newStatus = "rejected";
        notificationTitle = "Project Rejected";
        notificationMessage = `Your project "${project.title}" has been rejected. View detailed reviewer feedback and editor comments in your dashboard. ${comments || ""}`;
      } else {
        newStatus = "needs_revision";
        notificationTitle = "Revision Requested";
        notificationMessage = `Changes requested for your project "${project.title}". View detailed reviewer feedback and editor comments in your dashboard, then update your project. ${comments || ""}`;
      }
      ...
      await storage.logActivity({
        ...
        details: `${currentUser.firstName} ${currentUser.lastName} ${decision}ed project "${project.title}". ${comments ? `Comments: ${comments}` : ""}`,
      });

      res.json({ message: `Project ${decision}ed successfully`, status: newStatus });
    } catch (error) {
  ...
  app.get("/api/stats", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const pendingAssignment = allProjects.filter(p => p.status === 'pending_assignment').length;
      ...
      const avgScore = gradedProjects.length > 0
        ? gradedProjects.reduce((sum, p) => {
            const score = typeof p.finalScore === 'string' ? parseFloat(p.finalScore) : Number(p.finalScore);
            return sum + score;
      ...
      const totalReviewers = allUsers.filter(u => u.role === USER_ROLES.REVIEWER).length;
  ...
  app.post("/api/debug/retry-ai-grading", async (req, res) => {
    try {
      const allProjects = await storage.getAllProjects();
      const pendingProjects = allProjects.filter(p => p.status === 'pending_ai');
      
      if (pendingProjects.length === 0) {
        return res.json({ message: 'No projects pending AI grading', processed: 0 });
      }
  ...
  app.get("/api/debug/db-status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        userCount,
        projectCount,
        sampleUsers: users.map(u => ({ email: u.email, role: u.role, firstName: u.firstName, lastName: u.lastName })),
        sampleProjects: projects.map(p => ({ title: p.title, status: p.status, submitterId: p.submitterId }))
      });
    } catch (error) {
  ...
  app.get("/api/debug/editors", async (req, res) => {
    try {
      const { UserModel } = await import("./db");
      const editors = await UserModel.find({ role: 'editor' }).select('email firstName lastName role').lean();
  ...
  app.post("/api/admin/initialize-db", async (req, res) => {
    try {
      ...
      if (existingUsers > 0) {
        return res.status(400).json({ 
          message: "Database already has users. This endpoint can only be used on an empty database.",
      ...
      createdAccounts.push({ email: editor.email, password: "editor123", role: "Editor" });
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const reviewer = await storage.createUser({
          ...
          department: i === 1 ? "Computer Science" : i === 2 ? "Engineering" : "Business",
          specialty: i === 1 ? "AI & Machine Learning" : i === 2 ? "Software Engineering" : "Information Systems",
        });
        createdAccounts.push({ email: reviewer.email, password: "reviewer123", role: "Reviewer" });
      }
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const faculty = await storage.createUser({
          ...
          academicLevel: i === 1 ? "Professor" : i === 2 ? "Associate Professor" : "Assistant Professor",
          college: i === 1 ? "College of Computing" : i === 2 ? "College of Engineering" : "College of Business",
          department: i === 1 ? "Computer Science" : i === 2 ? "Civil Engineering" : "Management",
          specialty: i === 1 ? "Artificial Intelligence" : i === 2 ? "Structural Engineering" : "Strategic Management",
        });
        createdAccounts.push({ email: faculty.email, password: "faculty123", role: "Faculty" });
      }
