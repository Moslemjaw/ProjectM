...
export interface IStorage {
  ...
  createUser(user: Omit<UpsertUser, 'id'> & { id: string; password: string }): Promise<User>;
  ...
  // Project operations
  createProject(project: Omit<InsertProject, 'submitterId'> & { submitterId: string }): Promise<Project>;
  ...
  updateProject(id: string, updates: Partial<Omit<Project, 'id' | '_id' | 'createdAt' | 'submitterId'>>): Promise<void>;
  ...
  editorRejectProject(projectId: string, rejectionReason: string): Promise<void>;
  editorRequestRevision(projectId: string, revisionComments: string): Promise<void>;
  ...
  acceptReviewerAssignment(projectId: string, reviewerId: string): Promise<void>;
  rejectReviewerAssignment(projectId: string, reviewerId: string): Promise<void>;
  getAvailableProjectsForReviewer(reviewerId: string): Promise<ProjectWithRelations[]>;
  
  // Grading operations
  submitGrade(grade: Omit<InsertGrade, 'reviewerId'> & { reviewerId: string }): Promise<Grade>;
  getGrade(gradeId: string): Promise<Grade | undefined>;
  getGradesByProject(projectId: string): Promise<Grade[]>;
  getGradeByProjectAndEditor(projectId: string, reviewerId: string): Promise<Grade | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  private async ensureConnection() {
    ...
    if (!connected) {
      throw new Error('Database connection not available. Please check MongoDB Atlas IP whitelist.');
    }
  }
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    try {
      ...
      console.error('⚠️  Failed to save user to MongoDB:', (error as Error).message);
  ...
  async createUser(userData: Omit<UpsertUser, 'id'> & { id: string; password: string }): Promise<User> {
    try {
  ...
  // Project operations
  async createProject(projectData: Omit<InsertProject, 'submitterId'> & { submitterId: string }): Promise<Project> {
  ...
  async getProject(id: string): Promise<ProjectWithRelations | undefined> {
    ...
    // Get submitter
    const submitter = await UserModel.findOne({ id: project.submitterId }).lean();

    // Get reviewer assignments with reviewer details
    const assignments = await ReviewerAssignmentModel.find({ projectId: id }).lean();
    ...
    const gradeReviewers = await UserModel.find({ id: { $in: gradeReviewerIds } }).lean();
  ...
  async getProjectsBySubmitter(submitterId: string): Promise<ProjectWithRelations[]> {
    const projects = await ProjectModel.find({ submitterId }).sort({ createdAt: -1 }).lean();
    
    const projectsWithRelations = await Promise.all(
      projects.map(async (project) => {
        const submitter = await UserModel.findOne({ id: project.submitterId }).lean();
        
        const assignments = await ReviewerAssignmentModel.find({ projectId: project.id }).lean();
        const reviewerIds = assignments.map(a => a.reviewerId);
        const reviewers = await UserModel.find({ id: { $in: reviewerIds } }).lean();
        const reviewerAssignments = assignments.map(assignment => ({
          ...assignment,
        ...
        const gradeList = await GradeModel.find({ projectId: project.id }).lean();
        const gradeReviewerIds = gradeList.map(g => g.reviewerId);
        const gradeReviewers = await UserModel.find({ id: { $in: gradeReviewerIds } }).lean();
        const grades = gradeList.map(grade => ({
          ...grade,
  ...
  async getProjectsByReviewer(reviewerId: string): Promise<ProjectWithRelations[]> {
    const assignments = await ReviewerAssignmentModel.find({ reviewerId }).sort({ assignedAt: -1 }).lean();
    ...
    const projectsWithRelations = await Promise.all(
      projects.map(async (project) => {
        ...
        const projectAssignments = await ReviewerAssignmentModel.find({ projectId: project.id }).lean();
        const assignmentEditorIds = projectAssignments.map(a => a.reviewerId);
        const editors = await UserModel.find({ id: { $in: assignmentEditorIds } }).lean();
        ...
        const gradeEditors = await UserModel.find({ id: { $in: gradeEditorIds } }).lean();
        const grades = gradeList.map(grade => ({
          ...grade,
  ...
  async updateProject(id: string, updates: Partial<Omit<Project, 'id' | '_id' | 'createdAt' | 'submitterId'>>): Promise<void> {
  ...
  async updateProjectFinalScore(id: string, finalScore: string): Promise<void> {
    await ProjectModel.updateOne(
      { id },
      { finalScore: parseFloat(finalScore), status: 'graded', updatedAt: new Date() }
    );
  }
  ...
  async editorAcceptProject(projectId: string, reviewerIds: string[]): Promise<void> {
  ...
  async editorRejectProject(projectId: string, rejectionReason: string): Promise<void> {
  ...
  async editorRequestRevision(projectId: string, revisionComments: string): Promise<void> {
    const project = await ProjectModel.findOne({ id: projectId }).lean();
    if (!project) {
      throw new Error('Project not found');
    }
  ...
  async assignReviewers(projectId: string, reviewerIds: string[]): Promise<void> {
  ...
  async getReviewerAssignments(projectId: string): Promise<EditorAssignment[]> {
    const assignments = await ReviewerAssignmentModel.find({ projectId }).lean();
    return assignments as EditorAssignment[];
  }

  async acceptReviewerAssignment(projectId: string, reviewerId: string): Promise<void> {
  ...
  async rejectReviewerAssignment(projectId: string, reviewerId: string): Promise<void> {
    await ReviewerAssignmentModel.updateOne(
      { projectId, reviewerId },
  ...
  async getAvailableProjectsForReviewer(reviewerId: string): Promise<ProjectWithRelations[]> {
    ...
    const availableProjects = projectsWithRelations.filter(project => {
      const myAssignment = project.reviewerAssignments?.find((a: any) => a.reviewerId === reviewerId);
  ...
  // Grading operations
  async submitGrade(gradeData: Omit<InsertGrade, 'reviewerId'> & { reviewerId: string }): Promise<Grade> {
